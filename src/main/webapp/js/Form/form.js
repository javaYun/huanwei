	...
	if(tempImg!=undefined&&tempImg.length>0){
		...
		newImg.onload=function(){
            ...
            if(spanWidth<=16){
                ...
                if(dbInput4Att[0]&&dbInput4Att[0].currentStyle) {
...
function initFieldDisplay(jqField,isPrint,isForward){
    ...
    if(fieldVal.inputType!="attachment"&&fieldVal.inputType!="document"&&fieldVal.inputType!="image"&&fieldVal.inputType!="handwrite"
		    	...
		    	if(!isChromeIe11){
					...
					if(temp[0].scrollHeight >= 12 && !temp.hasClass("correlation_form_16")){
                        if(jqField.height()<temp[0].scrollHeight){
                        	if(fieldVal.inputType=='edocflowdealoption'&&jqField[0].children[0].innerText!=''){
		...
		}else if(editTag||jqField.hasClass(addClass) || fieldVal.inputType=="flowdealoption"){
			var s = jqField.find("span.xdRichTextBox");
			if(s.length>0&&s[0]!=null&&s[0].clientHeight<s[0].scrollHeight){
				jqField[0].children[0].style.height="auto";
			}
    ...
    switch(fieldVal.inputType){
    	...
    	case "exchangetask":
    		if(designTag||editTag){
                ...
                delSomePxWidth(tempInp,getPMBWidth(tempInp)+jqField.find(".ico16").outerWidth(true)+1,fieldVal);
            }else if(browseTag){
                delSomePxWidth(jqField.find("#"+idStr),null,fieldVal);
            }
    		break;
    			...
    			if(jqField.find(".comp").length>0){
    				...
    				if(compAttr){
    					...
    					if(tj.isShowImg!=undefined&&tj.isShowImg&&tj.type=="fileupload"&&jqField.find("img").length>0){
        					...
        					if(jqField.find("img").length<=0&&isPrint){//流程表关联底表带出人员照片，打印及打印预览图片不显示_20160226017343
        						tempCompField.comp();
        					}
        					delSomePxWidth(jqField.find("#"+idStr),null,fieldVal);
        				}
...
function calc(field, calcAll, calcSysRel) {
    ...
    if(inCalculate=="true" || inCondition=="true"){
        ...
        var url = _ctxPath + '/form/formData.do?method=calculate&formMasterId=' + $("#contentDataId",_mainBodyDiv).val() + '&formId=' + $("#contentTemplateId",_mainBodyDiv).val() + "&tableName=" + tableName + "&fieldName=" + fieldName + "&recordId=" + recordId + "&rightId="+$("#rightId",_mainBodyDiv).val() + "&calcAll=" + calcAll + "&calcSysRel=" + calcSysRel +"&moduleId="+$("#moduleId",_mainBodyDiv).eq(0).val()+"&tag=" + (new Date()).getTime();
        var formData = [];
        for ( var i = 0; i < form.tableList.length; i++) {
...
function showRelationList(field){
    ...
    var dialogUrl = ""
    var title = "";
    if(inputField.attr("formType")==1||inputField.attr("formType")==5||inputField.attr("formType")==6||inputField.attr("formType")==7){//流程表单
...
function selectOrgCallBack(e,options){
	...
	if(options&&options.hasRelationField){//有其他字段关联此字段
		...
		//xuker 20150405 start 增加一个属性用于保存多单位多部门的ID
	    if(e&&e.obj&&e.obj.length==1){
	        params['orgIds'] = e.obj[0].id;
	    }else if(e&&e.obj&&e.obj.length>1){
			var ids = "";
			for(var i=0;i< e.obj.length;i++){
				if(i== e.obj.length-1){
					ids+= e.obj[i].type +"|"+e.obj[i].id;
				}else{
					ids+= e.obj[i].type +"|"+e.obj[i].id+",";
				}
				//ids.push(e.obj[i]);
		...
		//xuker 20150405 end 增加一个属性用于保存多单位多部门的ID
...
function validateUrl(obj,param){
    ...
    var validateObj = $.parseJSON("{"+obj.attr("validate")+"}");
    var strRegex = /^(https|http|ftp|rtsp|mms)?:\/\/([0-9A-Za-z_-]+\.?)+(\/[0-9A-Za-z_-]+)*(\.[0-9a-zA-Z-_]*)?(:?[0-9]{1,4})?(\/[0-9A-Za-z_-]+\.?[0-9A-Za-z_-]+)*(\?([0-9a-zA-Z-_]+=[0-9a-zA-Z-_%]*(&|&amp;)?)*)?\/?$/; 
...
function validateBaseMark(obj,param){
	...
	var validateObj = $.parseJSON("{"+obj.attr("validate")+"}");
	if(validateObj.notNull&&param.checkNull){
	    if(value===""||value==="0"||value==="请选择公文文号"){
	        return false;
	    }
	}
    return true;
